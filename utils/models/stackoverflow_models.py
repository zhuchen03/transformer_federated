...
def scaled_dot_product_attention(query: tf.Tensor, key: tf.Tensor, value: tf.Tensor, mask: tf.Tensor):
  ...
  Args:
    query: shape == (..., seq_len_q, depth)  #add explanation of parameter
    key: shape == (..., seq_len_k, depth)
    value: shape == (..., seq_len_v, depth_v)
    mask: Float tensor with shape broadcastable
          to (..., seq_len_q, seq_len_k). Defaults to None.
  Returns:
    output:  #add explanation
    attention_weights:
  """
...
def positional_encoding(position, d_model):
  def get_angles(pos, i, d_model):
    angle_rates = 1 / np.power(10000, (2 * (i // 2)) / np.float32(d_model))  # Why 10000???
...
def create_transformer_lm(vocab_size=10000,
  ...
  extended_vocab_size = vocab_size + 3 + num_oov_buckets  # For pad/bos/eos/oov.
  inputs = tf.keras.layers.Input(shape=(None,))
  transformer = TransformerLM(num_layers, d_embed, d_model, num_heads, dff, extended_vocab_size,
                              max_position_encoding, rate=dropout)
  features = transformer(inputs)

  # use shared embedding by default. Have to put it here due to initialization of transformer.embedding.embeddings
